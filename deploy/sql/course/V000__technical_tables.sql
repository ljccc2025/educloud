-- EduCloud Course 数据库：技术表（迁移器引导 schema_migration_history，此处幂等保留同 DDL）
-- 依据：docs/superpowers/specs/2026-08-18-educloud-data-design.md 第 14 节「事件与幂等通用表」与 17.1「迁移历史与并发保护」。
-- 说明：actor_type 直接使用 VARCHAR(32)（M04 file V002/user V003 的加宽结果并入新库 V000，course 库不再需要单独的 widen 迁移）。
...
INSERT INTO outbox_sequence (source_name, `last_value`) VALUES ('educloud-course', 0);

-- Inbox：M05 暂无上游事件源，建表作为技术模板；启用时补充消费者。
...
-- HTTP 幂等记录：上传确认等写操作防重复提交（匿名 user_id 用 0 约定）。
...
-- 应用账号权限：audit_event 仅 INSERT/SELECT；其余技术表按业务需要授权（业务表在 V001 逐表授权）。
GRANT SELECT, INSERT, UPDATE, DELETE ON educloud_course.outbox_event TO 'course_app'@'%';
GRANT SELECT, UPDATE ON educloud_course.outbox_sequence TO 'course_app'@'%';
GRANT SELECT, INSERT, UPDATE, DELETE ON educloud_course.inbox_event TO 'course_app'@'%';
GRANT SELECT, INSERT ON educloud_course.audit_event TO 'course_app'@'%';
GRANT SELECT, INSERT, UPDATE, DELETE ON educloud_course.idempotency_record TO 'course_app'@'%';
