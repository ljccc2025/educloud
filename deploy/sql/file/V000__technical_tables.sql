-- EduCloud File 数据库：技术表（迁移器引导 schema_migration_history，此处幂等保留同 DDL）
...
INSERT INTO outbox_sequence (source_name, `last_value`) VALUES ('educloud-file', 0);

-- Inbox：M04 暂无上游事件源，建表作为技术模板；启用时补充消费者。
...
GRANT SELECT, INSERT, UPDATE, DELETE ON educloud_file.outbox_event TO 'file_app'@'%';
GRANT SELECT, UPDATE ON educloud_file.outbox_sequence TO 'file_app'@'%';
GRANT SELECT, INSERT, UPDATE, DELETE ON educloud_file.inbox_event TO 'file_app'@'%';
GRANT SELECT, INSERT ON educloud_file.audit_event TO 'file_app'@'%';
GRANT SELECT, INSERT, UPDATE, DELETE ON educloud_file.idempotency_record TO 'file_app'@'%';
