...
usage() {
  cat <<'USAGE'
Usage: bash deploy/scripts/provision-file-nacos.sh [--env-file PATH]

Provision the exact local Nacos namespace, user, role, and permissions required
by educloud-file (isolated from the Gateway identity). Set NACOS_ADMIN_USERNAME
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  case "$key" in
    ...
    NACOS_FILE_USERNAME) file_username="$value" ;;
    NACOS_FILE_PASSWORD) file_password="$value" ;;
  esac
done <"$env_file"
...
[[ "$file_username" == 'educloud_file' ]] || fail 'NACOS_FILE_USERNAME does not match educloud_file'
[[ "$file_password" =~ ^[A-Za-z0-9_.:@%+=-]{16,128}$ ]] || fail 'NACOS_FILE_PASSWORD is invalid'
...
cleanup() {
  unset admin_password file_password token NACOS_ADMIN_PASSWORD
...
users_response="$(nacos_call GET /v1/auth/users \
  "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' "username=$file_username")" || \
...
if [[ -z "$users" ]]; then
  nacos_call POST /v1/auth/users \
    "accessToken=$token" "username=$file_username" "password=$file_password" >/dev/null || \
    fail 'Unable to create Nacos File service user'
elif [[ "$users" != "$file_username" ]]; then
  fail 'Nacos File service user state is inconsistent'
fi

roles_response="$(nacos_call GET /v1/auth/roles \
  "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' \
  "username=$file_username" 'role=')" || fail 'Unable to read Nacos roles'
roles="$(python3 -c '
import json,sys
root=json.load(sys.stdin)
items=root.get("pageItems", []) if isinstance(root, dict) else []
...
if [[ -z "$roles" ]]; then
  nacos_call POST /v1/auth/roles \
    "accessToken=$token" "role=$file_username" "username=$file_username" >/dev/null || \
    fail 'Unable to bind the File service Nacos role'
elif [[ "$roles" != "$file_username" ]]; then
  fail 'File service Nacos user has an unexpected role binding'
fi
...
read_permissions() {
  ...
  response="$(nacos_call GET /v1/auth/permissions \
    "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' "role=$file_username")" || \
...
if [[ -n "$(comm -13 "$expected_permissions" "$actual_permissions")" ]]; then
  fail 'File service Nacos role has unexpected extra permissions'
fi
while IFS=$'\t' read -r action resource; do
  [[ -n "$action" ]] || continue
  nacos_call POST /v1/auth/permissions \
    "accessToken=$token" "role=$file_username" "resource=$resource" "action=$action" >/dev/null || \
    fail 'Unable to add a required Nacos permission'
done < <(comm -23 "$expected_permissions" "$actual_permissions")

read_permissions
cmp -s "$expected_permissions" "$actual_permissions" || fail 'Final File service Nacos permission set is inconsistent'

user_login="$(nacos_call POST /v1/auth/login \
  "username=$file_username" "password=$file_password")" || fail 'File service Nacos credential verification failed'
user_token="$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("accessToken", ""))' \
  <<<"$user_login" | tr -d '\r')"
[[ -n "$user_token" ]] || fail 'File service Nacos credential verification returned no token'
unset user_login user_token file_password token

printf 'Nacos File service namespace, identity, role, and exact permissions are ready.\n'
