...
while IFS= read -r line || [[ -n "$line" ]]; do
  if [[ "$line" =~ ^([A-Z0-9_]+)= ]]; then
    ...
    case "$variable_name" in
      MYSQL_ROOT_PASSWORD|EDUCLOUD_*_DB_PASSWORD|EDUCLOUD_*_MIGRATION_PASSWORD|REDIS_PASSWORD|RABBITMQ_DEFAULT_PASS|NACOS_AUTH_IDENTITY_VALUE|MINIO_ROOT_PASSWORD|GF_SECURITY_ADMIN_PASSWORD)
