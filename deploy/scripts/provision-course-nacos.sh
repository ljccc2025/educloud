...
usage() {
  cat <<'USAGE'
Usage: bash deploy/scripts/provision-course-nacos.sh [--env-file PATH]

Provision the exact local Nacos namespace, user, role, and permissions required
by educloud-course (isolated from the Gateway identity). Set NACOS_ADMIN_USERNAME
and optionally NACOS_ADMIN_PASSWORD. If the password is omitted it is read
without echo.
USAGE
}
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  case "$key" in
    ...
    NACOS_USER_NAMESPACE) namespace="$value" ;;
    NACOS_GATEWAY_DISCOVERY_GROUP) discovery_group="$value" ;;
    NACOS_COURSE_USERNAME) course_username="$value" ;;
    NACOS_COURSE_PASSWORD) course_password="$value" ;;
  esac
done <"$env_file"

[[ "$nacos_port" =~ ^[0-9]{1,5}$ ]] || fail 'NACOS_HTTP_PORT is invalid'
[[ "$namespace" == 'educloud-local' ]] || fail 'NACOS_USER_NAMESPACE does not match educloud-local'
[[ "$discovery_group" == 'EDUCLOUD_SERVICES' ]] || fail 'NACOS_GATEWAY_DISCOVERY_GROUP does not match EDUCLOUD_SERVICES'
[[ "$course_username" == 'educloud_course' ]] || fail 'NACOS_COURSE_USERNAME does not match educloud_course'
[[ "$course_password" =~ ^[A-Za-z0-9_.:@%+=-]{16,128}$ ]] || fail 'NACOS_COURSE_PASSWORD is invalid'
...
cleanup() {
  unset admin_password course_password token NACOS_ADMIN_PASSWORD
...
users_response="$(nacos_call GET /v1/auth/users \
  "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' "username=$course_username")" || \
...
if [[ -z "$users" ]]; then
  nacos_call POST /v1/auth/users \
    "accessToken=$token" "username=$course_username" "password=$course_password" >/dev/null || \
    fail 'Unable to create Nacos Course service user'
elif [[ "$users" != "$course_username" ]]; then
  fail 'Nacos Course service user state is inconsistent'
fi

roles_response="$(nacos_call GET /v1/auth/roles \
  "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' \
  "username=$course_username" 'role=')" || fail 'Unable to read Nacos roles'
roles="$(python3 -c '
import json,sys
root=json.load(sys.stdin)
items=root.get("pageItems", []) if isinstance(root, dict) else []
...
if [[ -z "$roles" ]]; then
  nacos_call POST /v1/auth/roles \
    "accessToken=$token" "role=$course_username" "username=$course_username" >/dev/null || \
    fail 'Unable to bind the Course service Nacos role'
elif [[ "$roles" != "$course_username" ]]; then
  fail 'Course service Nacos user has an unexpected role binding'
fi
...
printf 'r\t%s:%s:naming/educloud-course\n' "$namespace" "$discovery_group" >"$expected_permissions"
printf 'w\t%s:%s:naming/educloud-course\n' "$namespace" "$discovery_group" >>"$expected_permissions"
sort -u "$expected_permissions" -o "$expected_permissions"

read_permissions() {
  local response
  response="$(nacos_call GET /v1/auth/permissions \
    "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' "role=$course_username")" || \
...
if [[ -n "$(comm -13 "$expected_permissions" "$actual_permissions")" ]]; then
  fail 'Course service Nacos role has unexpected extra permissions'
fi
while IFS=$'\t' read -r action resource; do
  [[ -n "$action" ]] || continue
  nacos_call POST /v1/auth/permissions \
    "accessToken=$token" "role=$course_username" "resource=$resource" "action=$action" >/dev/null || \
    fail 'Unable to add a required Nacos permission'
done < <(comm -23 "$expected_permissions" "$actual_permissions")

read_permissions
cmp -s "$expected_permissions" "$actual_permissions" || fail 'Final Course service Nacos permission set is inconsistent'

user_login="$(nacos_call POST /v1/auth/login \
  "username=$course_username" "password=$course_password")" || fail 'Course service Nacos credential verification failed'
user_token="$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("accessToken", ""))' \
  <<<"$user_login" | tr -d '\r')"
[[ -n "$user_token" ]] || fail 'Course service Nacos credential verification returned no token'
unset user_login user_token course_password token

printf 'Nacos Course service namespace, identity, role, and exact permissions are ready.\n'