...
usage() {
  cat <<'USAGE'
Usage: bash deploy/scripts/provision-user-nacos.sh [--env-file PATH]

Provision the exact local Nacos namespace, user, role, and permissions required
by educloud-user (isolated from the Gateway identity). Set NACOS_ADMIN_USERNAME
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  case "$key" in
    ...
    NACOS_USER_USERNAME) user_username="$value" ;;
    NACOS_USER_PASSWORD) user_password="$value" ;;
  esac
done <"$env_file"
...
[[ "$user_username" == 'educloud_user' ]] || fail 'NACOS_USER_USERNAME does not match educloud_user'
[[ "$user_password" =~ ^[A-Za-z0-9_.:@%+=-]{16,128}$ ]] || fail 'NACOS_USER_PASSWORD is invalid'
...
cleanup() {
  unset admin_password user_password token NACOS_ADMIN_PASSWORD
...
users_response="$(nacos_call GET /v1/auth/users \
  "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' "username=$user_username")" || \
...
if [[ -z "$users" ]]; then
  nacos_call POST /v1/auth/users \
    "accessToken=$token" "username=$user_username" "password=$user_password" >/dev/null || \
    fail 'Unable to create Nacos User service user'
elif [[ "$users" != "$user_username" ]]; then
  fail 'Nacos User service user state is inconsistent'
fi

roles_response="$(nacos_call GET /v1/auth/roles \
  "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' \
  "username=$user_username" 'role=')" || fail 'Unable to read Nacos roles'
roles="$(python3 -c '
import json,sys
root=json.load(sys.stdin)
items=root.get("pageItems", []) if isinstance(root, dict) else []
...
if [[ -z "$roles" ]]; then
  nacos_call POST /v1/auth/roles \
    "accessToken=$token" "role=$user_username" "username=$user_username" >/dev/null || \
    fail 'Unable to bind the User service Nacos role'
elif [[ "$roles" != "$user_username" ]]; then
  fail 'User service Nacos user has an unexpected role binding'
fi
...
w	${namespace}:${discovery_group}:naming/educloud-user
EOF
...
read_permissions() {
  ...
  response="$(nacos_call GET /v1/auth/permissions \
    "accessToken=$token" 'pageNo=1' 'pageSize=100' 'search=accurate' "role=$user_username")" || \
...
if [[ -n "$(comm -13 "$expected_permissions" "$actual_permissions")" ]]; then
  fail 'User service Nacos role has unexpected extra permissions'
fi
while IFS=$'\t' read -r action resource; do
  [[ -n "$action" ]] || continue
  nacos_call POST /v1/auth/permissions \
    "accessToken=$token" "role=$user_username" "resource=$resource" "action=$action" >/dev/null || \
    fail 'Unable to add a required Nacos permission'
done < <(comm -23 "$expected_permissions" "$actual_permissions")

read_permissions
cmp -s "$expected_permissions" "$actual_permissions" || fail 'Final User service Nacos permission set is inconsistent'

user_login="$(nacos_call POST /v1/auth/login \
  "username=$user_username" "password=$user_password")" || fail 'User service Nacos credential verification failed'
user_token="$(python3 -c 'import json,sys; print(json.load(sys.stdin).get("accessToken", ""))' \
  <<<"$user_login" | tr -d '\r')"
[[ -n "$user_token" ]] || fail 'User service Nacos credential verification returned no token'
unset user_login user_token user_password token

printf 'Nacos User service namespace, identity, role, and exact permissions are ready.\n'
