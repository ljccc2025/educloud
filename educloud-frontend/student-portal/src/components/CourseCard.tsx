    ...
    >
      ...
      <div className="relative aspect-[16/10] overflow-hidden rounded-t-2xl bg-ink-100">
        ...
        {course.originalPrice && (
          <span className="absolute top-3 left-3 badge-amber">限时优惠</span>
        )}
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <p className="text-sm text-ink-500 mb-3 line-clamp-2 flex-1">{course.description}</p>
        ...
        <div className="flex items-center gap-2 mb-3 pb-3 border-b border-ink-50">
          <img
            ...
            className="w-7 h-7 bg-indigo-100 border border-ink-200"
          />
          <span className="text-sm text-ink-600">{course.teacherName}</span>
        </div>
        ...
        <div className="flex items-center gap-3 text-xs text-ink-400 mb-3">
          <span className="flex items-center gap-1">
            ...
            <span className="font-medium text-ink-700">{course.rating}</span>
          </span>
          <span className="flex items-center gap-1">
            <Users size={13} />
        ...
        <div className="flex items-baseline gap-2 mt-auto">
          {isFree ? (
            ...
            <>
              ...
              {course.originalPrice && (
                <span className="text-sm text-ink-300 line-through">¥{course.originalPrice}</span>
              )}
            </>
          )}
        </div>
