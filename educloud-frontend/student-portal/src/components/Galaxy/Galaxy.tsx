...
export default function Galaxy({
  ...
  useEffect(() => {
    ...
    try {
      const dprLimit = Number.isFinite(maxDpr) ? Math.max(1, maxDpr) : DEFAULT_MAX_DPR;
      const dpr = Math.min(window.devicePixelRatio || 1, dprLimit);
      const fpsLimit = Number.isFinite(maxFps) ? Math.max(1, maxFps) : DEFAULT_MAX_FPS;
      ...
      const isActive = () =>
        isIntersecting && isPageVisible && !reduceMotion && !disableAnimation;
      ...
      function tick(time: number) {
        ...
        if (elapsed + 0.5 >= frameInterval) {
          lastRenderAt = time - (elapsed % frameInterval);
      ...
      console.warn('Galaxy WebGL initialization failed; rendering the static page background.', error);
    }
