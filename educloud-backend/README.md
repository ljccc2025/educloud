...
此目录目前只有父 Maven 工程，用于锁定 Java 17 字节码目标、依赖版本和统一构建插件。构建 JDK 只接受 17 或 21；使用 JDK 21 构建时仍通过 `--release 17` 生成 Java 17 字节码。准备阶段不创建服务启动类、Controller、Entity、Mapper、占位接口或业务代码，也不声明尚未实现的 Maven 子模块。
