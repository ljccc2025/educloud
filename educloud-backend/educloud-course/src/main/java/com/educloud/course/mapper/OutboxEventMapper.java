...
public interface OutboxEventMapper extends BaseMapper<OutboxEventEntity> {
    ...
    @Update("UPDATE outbox_event SET publish_status = 'PENDING', "
            ...
            + "claim_owner = NULL WHERE id = #{id} AND publish_status = 'CLAIMED'")
    int markFailedAttempt(@Param("id") Long id, @Param("nextAttemptAt") Instant nextAttemptAt);

    /**
     * relay 投递失败且达重试阈值（终态语义保持本模块原有值）：CAS 置 FAILED、
     * attempt_count+1，next_attempt_at 一并保留供排查，不再认领（人工介入）。
     */
    @Update("UPDATE outbox_event SET publish_status = 'FAILED', "
            + "attempt_count = attempt_count + 1, next_attempt_at = #{nextAttemptAt}, "
            + "claim_owner = NULL WHERE id = #{id} AND publish_status = 'CLAIMED'")
    int markFailed(@Param("id") Long id, @Param("nextAttemptAt") Instant nextAttemptAt);
