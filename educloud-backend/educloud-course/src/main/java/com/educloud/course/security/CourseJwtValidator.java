...
/**
 * Course 服务 JWT claims 校验器（复制 file FileJwtValidator 适配 Course）。
 *
 * <p>用户令牌（aud 含 {@code educloud.course.jwt.audience}，如 educloud-api）执行严格
 * claims 契约：sub/sid/tokenVersion/userType/roles/permissions；服务令牌（aud 含
 * {@code educloud.course.internal.audience}，如 educloud-course）仅做时间/issuer 校验，
 * aud + clientId 白名单由 {@link com.educloud.course.config.InternalApiFilter} 自行完成
 * （与 M03/M04 模式一致，避免 Resource Server 的严格用户契约拒绝合法服务令牌）。</p>
 */
public final class CourseJwtValidator implements OAuth2TokenValidator<Jwt> {
    ...
    public CourseJwtValidator(CourseProperties properties, Clock clock) {
