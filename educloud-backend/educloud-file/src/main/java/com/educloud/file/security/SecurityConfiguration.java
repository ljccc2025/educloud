...
/**
 * File 服务安全配置：Resource Server（User 公钥 JWKS 验签）+ 方法安全。
 *
 * <p>依据：M04 设计规格第 9 节（对外 API 经 Gateway，Bearer + 权限码；File 再次验签）。
 * 全部请求 authenticated，permitAll 仅 /actuator/health/**；/internal/v1/** 由
 * {@link InternalApiFilter} 在 Security 链之前处理，服务令牌同样能通过 Resource Server
 * 解码（宽松分支），故无需单独放行。权限码（permissions claim）无前缀映射为 authority，
...
public class SecurityConfiguration {
     ...
     * 依据：M04 设计规格第 6.1 节（file:upload / file:storage:test 等权限码）。
     */
    @Bean
