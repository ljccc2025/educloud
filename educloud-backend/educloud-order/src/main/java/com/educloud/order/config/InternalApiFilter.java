...
public final class InternalApiFilter extends OncePerRequestFilter {
    ...
    public InternalApiFilter(ObjectProvider<JwtDecoder> jwtDecoderProvider, OrderProperties orderProperties) {
        this.jwtDecoderProvider = Objects.requireNonNull(jwtDecoderProvider, "jwtDecoderProvider");
        this.orderProperties = Objects.requireNonNull(orderProperties, "orderProperties");
    }
    ...
    protected void doFilterInternal(
        ...
        if (internalTokenHeader != null && !internalTokenHeader.isBlank()) {
            String configuredSecret = orderProperties.internal() != null ? orderProperties.internal().secretToken() : null;
        ...
        String effectiveAudience = orderProperties.internal() != null
                ? orderProperties.internal().effectiveInternalAudience()
                : "educloud-order";
        if (!(clientId instanceof String clientIdText)
                || !token.getAudience().contains(effectiveAudience)
                || (orderProperties.internal() != null
                    && !orderProperties.internal().effectiveAllowedClientIds().isEmpty()
                    && !orderProperties.internal().effectiveAllowedClientIds().contains(clientIdText))) {
