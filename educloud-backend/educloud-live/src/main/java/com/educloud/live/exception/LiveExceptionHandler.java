...
public final class LiveExceptionHandler {

    private final ApiResponseFactory responses;

    public LiveExceptionHandler(ApiResponseFactory responses) {
        this.responses = Objects.requireNonNull(responses, "responses");
    }

    @ExceptionHandler(LiveException.class)
    public ResponseEntity<ApiResponse<Object>> handleLiveException(LiveException exception) {
        return respond(exception.getErrorCode(), exception.getMessage(), null);
    }
    ...
    public ResponseEntity<ApiResponse<ValidationErrorDetails>> handleBodyValidation(
        ...
        List<FieldViolation> violations = exception.getBindingResult().getFieldErrors().stream()
                .map(LiveExceptionHandler::toViolation)
    ...
    private <T> ResponseEntity<ApiResponse<T>> respond(ErrorCode code, String message, T details) {
        ApiResponse<T> body = responses.error(code, message != null ? message : code.defaultMessage(), details);
    ...
    private static FieldViolation toViolation(FieldError error) {
        String code = error.getCode() != null ? error.getCode() : "Invalid";
