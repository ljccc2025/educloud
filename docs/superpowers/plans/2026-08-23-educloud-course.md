...
- [x] **步骤 1：写失败测试**：CourseContextTest 断言 \`CourseApplication\` 可加载上下文
- [x] **步骤 2：运行确认失败**：\`mvn -pl educloud-course -am test\` 预期 FAIL（模块不存在）
- [x] **步骤 3：实现骨架**：pom.xml/启动类/application.yml（端口 8089/8090；DB/Redis/Rabbit/Nacos 用 env 占位，本地测试 profile 用 H2 或关闭自动配置）
- [x] **步骤 4：运行测试确认通过**（context 加载绿）
- [x] **步骤 5：Commit**：\`git commit -m "feat(course): educloud-course 模块骨架与上下文测试"\`
...
- [x] **步骤 1：写失败测试**：断言 \`outbox_event\` 存在、\`inbox_event\` 唯一键 \`uk_inbox_event_id\`、audit_event.actor_type VARCHAR(32)
- [x] **步骤 2：运行确认失败**：\`mvn -pl educloud-course -am verify -Pintegration -Dtest=CourseSchemaIT\` 预期 FAIL
- [x] **步骤 3：编写 V000 SQL**（复制 file 版改 GRANT 账号；不写业务表）
- [x] **步骤 4：运行 IT 确认通过**
- [x] **步骤 5：Commit**：\`feat(course): educloud_course 技术表迁移与 Schema IT\`
...
- [x] **步骤 1：写失败测试**：断言 \`course\`/\`course_version\`/\`course_teacher\`/\`course_audit_submission\`/\`course_enrollment\`/\`course_content_readiness_projection\`/\`course_review\`/\`course_category\` 存在；\`course_version\` 唯一 (course_id, version_no)；\`course_enrollment\` 唯一 (course_id, student_id)；\`course_review\` 唯一 (course_id, student_id)
- [x] **步骤 2：运行确认失败**（V001 未写）
- [x] **步骤 3：编写 V001 DDL**（对照规格 5.2 字段与索引；version_status/lifecycle_status 用 VARCHAR + 应用层状态机校验）
- [x] **步骤 4：运行 IT 确认通过**
- [x] **步骤 5：Commit**：\`feat(course): educloud_course 业务表迁移与 Schema IT\`
...
- [x] **步骤 1：写失败测试**（断言 course:audit 权限码、COURSE_REVIEWER 角色、关联行存在；重复执行 V004 不报错）
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：编写 V004 SQL**（INSERT ... ON DUPLICATE KEY UPDATE 幂等；角色-权限关联同样幂等）
- [x] **步骤 4：运行 IT 确认通过**
- [x] **步骤 5：Commit**：\`feat(course): user 库 V004 course 权限码与 COURSE_REVIEWER 角色\`
...
- [x] **步骤 1：写失败测试**：MapperIT 断言 insert/select 往返（Testcontainer）；乐观锁版本回写测试（update 后 version 递增且**不手动 +1**）
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现实体/Mapper/Config**（@TableName/@TableId(ASSIGN_ID)/@Version 仅 course 与 enrollment；表名与 V001 对齐）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 实体 Mapper 与 MyBatis-Plus 配置\`
...
- [x] **步骤 1：写失败测试**（每个错误码 code/httpStatus 断言）
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（对齐 common 的 ErrorCode 接口模式）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 错误码与全局异常处理\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现安全配置**（JWKS 路径 env 默认 \`classpath:jwks-test.json\` 供测试；测试 profile 生成测试密钥对——参照 user/file 测试做法）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): JWT Resource Server 与内部过滤器\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（查询 status=VISIBLE 全量，内存组树按 sort_order）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 课程分类查询\`
...
- [x] **步骤 1：写失败测试**：POST /courses 建根+首版 DRAFT；PUT 只允许 DRAFT（VERSION_NOT_DRAFT）；跨教师访问草稿 403；POST drafts 从 PUBLISHED/REJECTED 复制新草稿并 version_no+1
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（建课时事务插入 course（owner=当前教师）+ course_teacher(OWNER) + course_version(DRAFT)；PUT 校验归属 + version_status=DRAFT + 乐观锁；封面 bind 见任务 12）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 课程创建与草稿管理\`
...
- [x] **步骤 1：写失败测试**（状态机全转移 + 非法转移；approve 同事务断言 course.published_version_id 切换）
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（submit：校验归属+DRAFT→PENDING_REVIEW+写 submission；approve：SELECT course FOR UPDATE→submission APPROVED→published_version_id 切换→旧版 SUPERSEDED→lifecycle=PUBLISHED→outbox CoursePublished→同事务提交；reject 原因必填；withdraw 仅提交教师+PENDING）
- [x] **步骤 4：运行确认通过**（含 IT）
- [x] **步骤 5：Commit**：\`feat(course): 课程审核状态机与原子发布\`
...
- [x] **步骤 1：写失败测试**：offline 仅 PUBLISHED；republish 仅 OFFLINE 且有 published_version_id（M05 就绪 gate 恒放行）；archive 仅 OFFLINE（PUBLISHED 直接 archive → 409）、归档后不可 republish；权限校验
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（锁根 + 状态迁移 + outbox CourseOfflined/CourseRepublished/CourseArchived）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 课程生命周期操作\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（SQL 分页 + categoryId/level/priceRange/keyword 过滤 + 排序白名单映射；detail 按可见性：PUBLISHED 公开 / 归属教师看 draft/pending；review 列表分页）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 课程公开列表与详情\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现 FileClient + 注册**（File 侧 OwnerServiceRegistry 加 course clientId；File 现有 IT 全量回归）
- [x] **步骤 4：运行确认通过**（course + file 模块测试全绿）
- [x] **步骤 5：Commit**：\`feat(course): 封面 File 集成（bind/批量 grant）\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（选课：锁根→校验 PUBLISHED+免费→幂等 INSERT IGNORE/已存在返回→count 递增→outbox EnrollmentCreated；我的课程：enrollment JOIN course JOIN published version；学生列表：归属校验 + 分页）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 免费选课/我的课程/学生列表\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（upsert：校验 ACTIVE 选课→INSERT ON DUPLICATE→同事务重算 rating_avg/rating_count；DELETE /course-reviews/{id}：管理角色→HIDDEN→重算）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 课程评价与评分汇总\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（复用 common OutboxWriter；分发线程 + 退避 + FAILED 阈值）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 课程/选课事件与 Outbox 分发\`
...
- [x] **步骤 1：写失败测试**（指标计数递增、审计行写入与 actor_type 长度兼容 VARCHAR(32)）
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（复用 M04 模式：CompositeHealthIndicator + Micrometer counter + audit_event 写库）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：`feat(course): 可观测性（readiness/业务指标/审计）`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：实现**（InternalApiFilter 校验 aud=educloud-course + clientId 白名单；返回可见性/归属快照，供 M06 content 消费）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 内部课程访问接口\`
...
- [x] **步骤 1：写失败测试**
- [x] **步骤 2：运行确认失败**
- [x] **步骤 3：编写种子 SQL**（Snowflake 用固定可读 ID 如 9000000000000000100+ 序列，避免与运行时 ID 冲突；slug 唯一）
- [x] **步骤 4：运行确认通过**
- [x] **步骤 5：Commit**：\`feat(course): 演示种子数据（分类/课程/选课/评价）\`
...
- [x] **步骤 1：本地验证**：\`mvn -pl educloud-course -am package -DskipTests\` 成功
- [x] **步骤 2：Commit**（\`chore(deploy): educloud-course 部署脚本与环境变量\`）
...
- [x] **步骤 2：迁移**：\`MYSQL_HOST=127.0.0.1 MYSQL_PORT=3306 EDUCLOUD_COURSE_MIGRATION_PASSWORD=… bash deploy/scripts/run-migrations.sh --service course\` + user 库 V004
- [x] **步骤 3：Nacos provision**：\`bash deploy/scripts/provision-course-nacos.sh --env-file deploy/docker-compose/.env\`
- [x] **步骤 4：bootstrap**：\`CLIENT_ID=educloud-course AUDIENCES='["educloud-file"]' SCOPES='["file:internal"]' BOOTSTRAP_KEY=… printf '%s' "$SECRET" | bash deploy/scripts/bootstrap-service-clients.sh\`
- [x] **步骤 5：构建**：\`/opt/maven/bin/mvn -pl educloud-course -am package -DskipTests\`（VM JDK17）+ 前端 \`tsc && vite build\`
- [x] **步骤 6：启动**：\`bash deploy/scripts/start-dev.sh\`（course 8089/8090 监听；gateway 日志确认 course-core 路由可达）
- [x] **步骤 7：链路验证**（curl/python 走网关）：登录 demo_teacher → 建课 → 编辑草稿 → 提交 → demo_admin 审批 → 学生 fe_demo_10 列表可见新课程；免费选课幂等（重复 200）；未选课学生写评价 403；匿名 GET /courses 无封面泄露；越权：教师 A 读教师 B 草稿 403、学生查他人学生列表 403、伪造 coverFileId bind 403
- [x] **步骤 8：浏览器验证**：学生端列表/详情/选课/我的课程/评价；教师端建课→上传封面→提交；管理端审批；三门户状态一致
...
- [x] **步骤 1：typecheck 基线**（\`npx tsc --noEmit\` 通过）
- [x] **步骤 2：实现 courseApi.ts + types 对齐**
- [x] **步骤 3：替换 CourseList/CourseDetail/MyCourses 数据源**（无 mock 回退；加载/空/错误 UI 保留）
- [x] **步骤 4：浏览器冒烟**（VM：登录→列表→详情→选课→我的课程→评价）
- [x] **步骤 5：Commit**（\`feat(student): 课程模块真实 API 联调\`）
...
- [x] **步骤 4：浏览器冒烟**（建课→填表→封面上传→保存草稿→提交审核；学生列表；留待最终 Playwright 统一执行）
...
- [x] **步骤 4：浏览器冒烟**（教师提交→admin 批准→学生端出现；驳回原因必填；留待最终 Playwright 统一执行）
...
- [x] **步骤 1：全量**：\`mvn -pl educloud-common,educloud-gateway,educloud-user,educloud-file,educloud-course -am verify\`（-Pintegration 跑 IT）BUILD SUCCESS
- [x] **步骤 2：规格审查**：对照 2026-08-23-educloud-course-design.md 逐节核对实现与测试覆盖
- [x] **步骤 3：独立代码审查**：按 chinese-code-review 覆盖六维度；重点：越权（跨教师草稿/学生列表/伪造封面/自审）、状态机非法转移、选课幂等与并发、评价范围、审核原子性、事件版本一致性；自动修复可确定项，其余列待确认
- [x] **步骤 4：修复验证 + 汇报**（含 BUG 风险表）→ 等待用户确认后进入 M06
