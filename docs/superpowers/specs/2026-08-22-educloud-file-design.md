...
- [ ] 读取 2026-08-18 领域/安全/数据/API/事件/测试/部署文档并审阅 diff
- [ ] 先写失败测试并确认失败
- [ ] 实现最小后端能力（模块测试绿）
- [ ] 集成测试（Testcontainer MinIO）绿
- [ ] 全量 `mvn verify` 绿
- [ ] 规格审查与质量审查（含独立代码审查：越权/类型伪造/超限/未绑定访问测试通过）
- [ ] VM 部署：迁移 `educloud_file`、bootstrap `user-service` 客户端、JWKS 配置、start-dev 拉起
- [ ] 浏览器验证：三门户头像上传/展示闭环 + 越权用例（他人 fileId 不可访问）
- [ ] 向用户汇报并等待确认后进入 M05
